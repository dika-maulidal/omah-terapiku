...
<head>
    ...
    <title>Bukti Pendaftaran - {{ $pendaftaran->nama }} ({{ $pendaftaran->kode_pendaftaran }})</title>
    <link rel="icon" type="image/png" sizes="16x16" href="{{ asset('images/logo.png') }}">
    <link rel="stylesheet" href="{{ asset('vendor/bootstrap/dist/css/bootstrap.min.css') }}">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"></script>
    <style>
        /* Format Dokumen Rekam Medis & Pendaftaran Standar Kedinasan (Monokrom / Standar Reference PDF) */
        @page {
            size: A4 portrait;
...
@php
    ...
    $usiaTahun = $safeAge($pendaftaran->tgl_lahir);
    $tglDaftarFormatted = $safeFormatDate($pendaftaran->created_at, date('d F Y'));
    $tglCetak = \Carbon\Carbon::now()->translatedFormat('d F Y, H:i') . ' WIB';

    $uptNama = $pendaftaran->upt_lokasi ?: ($upt ? $upt->nama : 'UPT RSBN Malang');
    ...
    <div class="container-fluid d-flex flex-wrap align-items-center justify-content-between" style="max-width: 820px; gap: 8px; padding: 0;">
        <div class="d-flex align-items-center flex-wrap" style="gap: 6px;">
            <span class="badge badge-primary font-w600" style="font-size: 11px; padding: 4px 8px; background: #2563eb; color: #fff;">
                <i class="fa-solid fa-file-invoice mr-1"></i> BUKTI PENDAFTARAN
            </span>
            <strong style="color: #0f172a; font-size: 12.5px;">{{ $pendaftaran->nama }}</strong>
            <span class="text-muted" style="font-size: 11.5px;">({{ $pendaftaran->kode_pendaftaran }})</span>
        ...
        <div class="d-flex align-items-center flex-wrap" style="gap: 6px;">
            ...
            @if(Session::has('portal_pasien_id'))
                <a href="{{ route('portal.dashboard') }}" class="btn btn-sm btn-light border font-w600" style="padding: 5px 10px; font-size: 11.5px; border-radius: 4px; color: #334155;">
...
<div class="print-container" id="print-area">
    ...
    <!-- Header Box Judul & Nomor Registrasi -->
    <table class="table-assessment header-box-table" style="width: 100%; border-collapse: collapse; margin-bottom: 6px;">
        <tr>
            <td style="width: 58%; text-align: center; vertical-align: middle; border: 1.5px solid #000000 !important; padding: 6px 8px; background: #ffffff;">
                <div style="font-size: 11pt; font-weight: 800; letter-spacing: 0.5px; text-transform: uppercase;">BUKTI PENDAFTARAN PENERIMA MANFAAT BARU</div>
                <div style="font-size: 9.5pt; font-weight: bold; text-transform: uppercase; margin-top: 1px;">(SISTEM REGISTRASI ONLINE TERPADU)</div>
            ...
            <td style="width: 42%; border: 1.5px solid #000000 !important; padding: 0; vertical-align: top; background: #ffffff;">
                <table style="width: 100%; border-collapse: collapse; font-size: 8.5pt;">
                    <tr>
                        <td style="border: none !important; border-bottom: 1px solid #000000 !important; padding: 3px 6px; font-weight: bold; width: 44%;">KODE REGISTRASI</td>
                        <td style="border: none !important; border-bottom: 1px solid #000000 !important; padding: 3px 6px; font-weight: 800; font-size: 9.5pt; letter-spacing: 0.5px;">: {{ $pendaftaran->kode_pendaftaran }}</td>
                    </tr>
                    <tr>
                        <td style="border: none !important; border-bottom: 1px solid #000000 !important; padding: 3px 6px; font-weight: bold;">Tgl. Mendaftar</td>
                        <td style="border: none !important; border-bottom: 1px solid #000000 !important; padding: 3px 6px;">: {{ $tglDaftarFormatted }}</td>
                    </tr>
                    <tr>
                        <td style="border: none !important; padding: 3px 6px; font-weight: bold;">Status Berkas</td>
    ...
    <table class="table-assessment" style="margin-bottom: 6px; font-size: 8.5pt;">
        <tbody>
            <tr>
                ...
                <td style="width: 30%;">: <strong>{{ $pendaftaran->nama }}</strong></td>
                <td style="width: 20%; font-weight: bold;">Nama Orang Tua / Wali</td>
                <td style="width: 30%;">: {{ $pendaftaran->nama_wali ?: '-' }} ({{ $pendaftaran->hubungan_wali ?: 'Wali' }})</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Nomor NIK Pasien</td>
                <td style="font-weight: bold;">: {{ $pendaftaran->nik ?: '-' }}</td>
                <td style="font-weight: bold;">No. BPJS / KIS</td>
                <td>: {{ $pendaftaran->no_bpjs ?: '-' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">TTL / Usia Pasien</td>
                <td>: {{ $pendaftaran->tmp_lahir ? $pendaftaran->tmp_lahir . ', ' : '' }}{{ $safeFormatDate($pendaftaran->tgl_lahir) }} ({{ $usiaTahun }})</td>
                <td style="font-weight: bold;">No. Kontak / WhatsApp</td>
                <td style="font-weight: bold;">: {{ $pendaftaran->no_hp }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Jenis Kelamin</td>
                <td>: {{ $pendaftaran->jk ?: '-' }}</td>
                <td style="font-weight: bold;">Desil Kesejahteraan</td>
            ...
            <tr>
                ...
                <td>: {{ $pendaftaran->alat_bantu ?: 'Tanpa Alat Bantu (Mandiri)' }}</td>
            </tr>
            <tr>
                <td style="font-weight: bold;">Alamat Domisili</td>
                <td colspan="3">
                    : {{ $pendaftaran->alamat_lengkap ?: '-' }}
                    @if($pendaftaran->kelurahan || $pendaftaran->kecamatan || $pendaftaran->kabupaten)
                        (Kel: {{ $pendaftaran->kelurahan ?: '-' }}, Kec: {{ $pendaftaran->kecamatan ?: '-' }}, Kab/Kota: {{ $pendaftaran->kabupaten ?: '-' }} {{ $pendaftaran->kodepos ? ' - '.$pendaftaran->kodepos : '' }})
                    @endif
                </td>
            </tr>
        </tbody>
    ...
    <!-- =========================================================================
         II. LAYANAN TERAPI & RENCANA KUNJUNGAN PERTAMA
         ========================================================================= -->
    <div class="section-title">
        II. LAYANAN TERAPI &amp; RENCANA KUNJUNGAN PERTAMA
    </div>
    <table class="table-assessment" style="margin-bottom: 6px; font-size: 8.5pt;">
        <tbody>
            <tr>
                ...
                <td style="width: 30%;">: <strong>{{ $pendaftaran->layanan_terapi ?: 'Layanan Terapi Terpadu' }}</strong></td>
                <td style="width: 20%; font-weight: bold;">Lokasi UPT Pelayanan</td>
            ...
            <tr>
                ...
                <td colspan="3">: {{ $pendaftaran->keluhan_utama ?: '-' }}</td>
            </tr>
        </tbody>
    </table>

    <!-- =========================================================================
         III. PENETAPAN PETUGAS & PENUGASAN TERAPIS (JIKA DISETUJUI / DITOLAK)
         ========================================================================= -->
    @if($pendaftaran->status === 'disetujui')
        <div class="section-title">
            III. PENETAPAN NO. REKAM MEDIS &amp; PENUGASAN PETUGAS MEDIS
        </div>
        <table class="table-assessment" style="margin-bottom: 6px; font-size: 8.5pt;">
            <tbody>
                <tr>
                    <td style="width: 20%; font-weight: bold;">No. Rekam Medis (No. RM)</td>
                    <td style="width: 30%; font-weight: 800; font-size: 9.5pt;">: {{ $pendaftaran->no_rm_diterbitkan }}</td>
                    <td style="width: 20%; font-weight: bold;">Terapis Penanggung Jawab</td>
                    <td style="width: 30%;">: <strong>{{ $pendaftaran->terapis ? $pendaftaran->terapis->nama : 'Petugas Medis Omah Terapi-KU' }}</strong></td>
                </tr>
                <tr>
    ...
    @elseif($pendaftaran->status === 'ditolak')
        <div class="section-title">
            III. CATATAN PENOLAKAN PENDAFTARAN
        </div>
        <div class="box-rekap" style="border-color: #000000; background-color: #fafafa;">
            <strong>Catatan Petugas:</strong> {{ $pendaftaran->catatan_petugas ?: 'Berkas atau persyaratan pendaftaran belum memenuhi kriteria layanan.' }}
        </div>
    @endif

    <!-- =========================================================================
         PETUNJUK & KETENTUAN ALUR KEHADIRAN
         ========================================================================= -->
    <div class="box-rekap no-break" style="margin-top: 6px; font-size: 8pt; line-height: 1.35;">
        <strong><i class="fa-solid fa-circle-info mr-1"></i> Petunjuk &amp; Ketentuan Bagi Orang Tua / Wali Pemohon:</strong>
        <ol style="margin: 3px 0 0 0; padding-left: 18px;">
            <li>Simpan lembar bukti pendaftaran ini dengan baik sebagai dokumen resmi pengajuan layanan Omah Terapi-KU.</li>
            <li>Pantau perkembangan verifikasi berkas atau penugasan terapis secara realtime pada portal online melalui tab <strong>Lacak Status</strong> menggunakan Kode Registrasi <strong>{{ $pendaftaran->kode_pendaftaran }}</strong> dan No. HP Anda.</li>
            <li>Setelah pendaftaran berstatus disetujui resmi, hadir tepat waktu sesuai jadwal sesi dengan membawa fotokopi Kartu Keluarga (KK) dan Surat Rujukan / Resume Medis bila ada.</li>
        </ol>
    </div>
        ...
        <table class="sign-table">
            <tr>
                <td style="width: 50%;">
                    ...
                    <div class="sig-name">{{ $pendaftaran->nama_wali ?: $pendaftaran->nama }}</div>
                    <div style="font-size: 7.5pt; color: #444;">Tanda Tangan Pemohon</div>
                </td>
                <td style="width: 50%;">
                    <div>Jawa Timur, {{ date('d F Y') }}</div>
                    <div>Petugas Verifikator / Pelayanan Omah Terapi-KU</div>
                    <div class="sig-spacer"></div>
                    <div class="sig-name">{{ $pendaftaran->verifier ? $pendaftaran->verifier->name : 'Petugas Omah Terapi-KU' }}</div>
...
<script>
    ...
    function downloadPDF() {
        ...
        var opt = {
            margin:       [8, 10, 10, 10],
            filename:     'Bukti_Pendaftaran_{{ $pendaftaran->kode_pendaftaran }}_{{ preg_replace("/[^a-zA-Z0-9]/", "_", $pendaftaran->nama) }}.pdf',
